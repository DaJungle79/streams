...
- **Daemon, `streams.daemon`.** `run_poll_tick` (frequent: capture + reconcile note edits + reminder completions + iMessage replies) and `run_scheduled_pass` (ingest → two-layer agent → project synthesis back to notes → nudge digest → health), both taking a `Deps` bundle so tests drive them with fakes. `run_forever` is the thin launchd-kept loop (catches per-tick errors). `health_check` probes each bridge; `streams.report` aggregates the `meta` cost/noise ledger; `streams setup` writes `config.yaml`. See `docs/ops.md`.
