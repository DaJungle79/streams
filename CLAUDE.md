...
- **Note identity & capture.** A stream's note is identified by `note_id` (stored in `stream.md`), set when the note is created — that mapping is the source of truth for which note is which. Discovery is by **folder**: a configurable Apple Notes folder (`note_folder`, default `Streams`) holds all managed notes, and `sync.capture_folder` adopts any note in that folder whose id we don't track into a new stream (user creates a note in the Streams folder → it becomes a stream). Folder membership is used because native Apple `#hashtags` are *not* part of a note's AppleScript-readable `body`, so they can't be discovered — the folder is queried reliably via `find_notes_in_folder`. The note is created lazily (in the folder) by `sync_stream` on first sync (keeping the store Apple-free); `stream create --note` opts into immediate creation.
